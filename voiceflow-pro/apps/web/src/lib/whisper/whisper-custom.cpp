//
// Custom Javascript API for whisper.cpp with proper output
// Based on the original emscripten.cpp but with transcription output
//
...
EMSCRIPTEN_BINDINGS(whisper) {
    ...
    emscripten::function("full_default", emscripten::optional_override([](const emscripten::val & audio, const std::string & lang, bool translate) {
        ...
        // run whisper
        {
            ...
            if (whisper_full(g_context, params, pcmf32.data(), pcmf32.size()) != 0) {
                return -2;
            }
        ...
        // IMPORTANT: Output the actual transcription!
        {
            ...
            for (int i = 0; i < n_segments; ++i) {
                ...
                const int64_t t0 = whisper_full_get_segment_t0(g_context, i);
                const int64_t t1 = whisper_full_get_segment_t1(g_context, i);
